function q = curve_to_equals_lines(p,N)

% curve_to_equals_lines Evenly spaced points along an existing curve in 2D or 3D.
%   curve_to_equals_lines(P,N) generates N points that interpolates a curve
...
for k0 = 1:len-1
   dist_bet_pts(k0) = distance_between_points(p(k0,:),p(k0+1,:));
end
...
for k = 1:N-1
   ...
   while isempty(newpt)
      ...
      % closest point in p
      disttmp = distance_between_points(ptnow,pttarget);
