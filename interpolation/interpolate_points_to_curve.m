function [pt,dudt,fofthandle] = interpolate_points_to_curve(t,px,py,varargin)
% interparc: interpolate points along a curve in 2 or more dimensions
% usage: pt = points_to_curve(t,px,py)    % a 2-d curve
% usage: pt = points_to_curve(t,px,py,pz) % a 3-d curve
% usage: pt = points_to_curve(t,px,py,pz,pw,...) % a 4-d or higher dimensional curve
% usage: pt = points_to_curve(t,px,py,method) % a 2-d curve, method is specified
